...
#   mixed: nodes on R580 / CUDA 13.0 run the cu130 image NATIVELY, while older
#   nodes (R575 / CUDA 12.9, R570 / CUDA 12.8) need the forward-compat libs the
#   base image ships at /usr/local/cuda/compat. We probe the native driver and
#   prepend compat ONLY when the host actually needs it -- forcing compat on a
#   driver that is already new enough loads an OLDER user-mode libcuda than the
...
# contention. Verified against the SGLang 0.5.18 source (carried over to
# 0.5.19):
...
# which requires a writable PROMETHEUS_MULTIPROC_DIR. Verified in 0.5.18
# (unchanged in 0.5.19):
...
# CUDA the IMAGE was built for, encoded as 1000*major + 10*minor (cu130 -> 13000).
# This is a property of THIS image's base (FROM ...cu130..., CUDA 13.0.3), not a
# node guess; bump it only when the base image's CUDA changes.
IMAGE_CUDA_VERSION="${IMAGE_CUDA_VERSION:-13000}"
export IMAGE_CUDA_VERSION
_cuda_probe() {
    python3 - <<'PY'
import ctypes, os, sys
need = int(os.environ.get("IMAGE_CUDA_VERSION", "13000"))
try:
...
if _cuda_probe; then
    ...
    _compat_applied=0
    for COMPAT in /usr/local/cuda/compat /usr/local/cuda-13.0/compat /usr/local/cuda-13/compat; do
...
# --- 8a. Debug Dump (OPT-IN) ---
# The old always-on dump ran nvidia-smi/ldconfig/du + ~10 Python imports
# (torch, flashinfer, triton, ...) + TWO full `--help` crawls of the SGLang
# argparsers — each help call imports the whole framework, adding minutes to
# every boot (painful in CrashLoopBackOff where it fires per restart).
# It is now opt-in: set DEBUG_DUMP=1 when post-mortem detail is actually
# needed; default OFF for fast, quiet boots. The pre-exec summary line above
if [[ "${DEBUG_DUMP:-0}" == "1" ]]; then
    _debug_dump_main() {
        echo "[debug] ===== STARTUP DEBUG DUMP @ $(date -u '+%Y-%m-%dT%H:%M:%SZ') ====="
        echo "[debug] identity: $(id 2>/dev/null) hostname=$(hostname) bash=${BASH_VERSION:-?}"
        echo "[debug] env (secrets redacted):"
        ...
        echo "[debug] TP_ARGS=[${TP_ARGS[*]:-}] SERVED_ARGS=[${SERVED_ARGS[*]:-}] EXTRA_ARGS_ARR=[${EXTRA_ARGS_ARR[*]:-}]"
        echo "[debug] gpu:"; nvidia-smi 2>/dev/null | head -30 | sed 's/^/[debug]   /'
        echo "[debug] disk:"; df -h /tmp /dev/shm "${BASE_PATH:-/}" 2>/dev/null | sed 's/^/[debug]   /'
        echo "[debug] model dir:"; ls -lah "${MODEL_DOWNLOAD_FOLDER:-}" 2>/dev/null | head -20 | sed 's/^/[debug]   /'
        echo "[debug] versions:"; python3 -c 'import sglang; print("sglang", sglang.__version__)' 2>&1 | sed 's/^/[debug]   /'
        echo "[debug] cache writability:"
        for _d in "${SGLANG_CACHE_DIR}" "${TORCHINDUCTOR_CACHE_DIR}" "${CUDA_CACHE_PATH}" "${HOME}" "${TMPDIR}"; do
            [ -w "${_d}" ] && echo "[debug]   ${_d}: WRITABLE" || echo "[debug]   ${_d}: NOT-WRITABLE/MISSING"
        done
        echo "[debug] ===== end dump ====="
    }
    _debug_dump_main
...
if [ "${MANIFEST_BASENAME}" = "model_index.json" ]; then
    ...
    # in launch_server's argparser on 0.5.18 (unchanged in 0.5.19).
    LAUNCH_ARGS=(
      --host=0.0.0.0
      --port=8080
      --model-path="${MODEL_DOWNLOAD_FOLDER}"
