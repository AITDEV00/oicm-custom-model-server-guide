...
# contention. Verified against the SGLang 0.5.16 source:
...
# which requires a writable PROMETHEUS_MULTIPROC_DIR. Verified in 0.5.16
...
if [ "${MANIFEST_BASENAME}" = "model_index.json" ]; then
    ...
    # in launch_server's argparser on 0.5.16.
    LAUNCH_ARGS=(
      --host=0.0.0.0
      --port=8080
      --model-path="${MODEL_DOWNLOAD_FOLDER}"
