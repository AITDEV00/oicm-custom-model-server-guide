...
(EXTRA_ARGS) is turned into the argv SGLang actually receives.

Used by:
  - startup.sh section 7 (subprocess: `python3 -m arg_normalizer --argv-nul`)
...
Goals:
  ...
  * Auto-fix what's recoverable: protect unquoted JSON before shlex strips its
    inner ", dedup repeated flags last-wins (argparse-style).
...
# SGLang grows new ones every release, and any such list will drift behind the
# version actually running in the image. The JSON-valued flags in SGLang 0.5.18
# (unchanged in 0.5.19; verified against `python3 -m sglang.launch_server --help`)
# include:
#   --json-model-override-args      (a JSON dict to override model config defaults)
#   --model-loader-extra-config     (JSON config for the model loader)
#   --kv-events-config              (JSON for NVIDIA dynamo KV event publishing)
#   --preferred-sampling-params     (type=loads -- explicitly JSON-parsed)
#
# (cu130 variant: identical SGLang 0.5.19 code as the cu129 image; only the
# bundled CUDA toolkit differs, which does not touch any flag parsing here.)
# Note SGLang's speculative decoding is NOT a JSON blob: it is a set of scalar
# flags (--speculative-algorithm, --speculative-num-steps, --speculative-eagle-topk,
# --speculative-num-draft-tokens), which the normalizer routes through the normal
# scalar path automatically.
#
# Instead of maintaining that list, we detect JSON values STRUCTURALLY: a token
# whose value starts with '{' or '['. That's the only thing that matters for the
# two failure modes this normalizer fixes (shlex stripping the inner " from an
# unquoted JSON, and an operator hand-writing malformed JSON). It works for every
# current SGLang flag AND every future one, with no list to maintain.
...
def normalize(raw):
    ...
    Returns a dict with keys:
      ...
      tokens         : list of normalized argv tokens (what SGLang would receive)
...
# ---------------------------------------------------------------------------
# CLI: called by startup.sh section 7 for the in-report verdict
# ---------------------------------------------------------------------------

def _render_text_report(result):
    lines = []
    line = lines.append
    line("  raw EXTRA_ARGS+SGLANG_EXTRA_ARGS : %r" % result["raw"])
    ...
    line("")
    line("  FINAL argv -> what production hands to SGLang (%d tokens):" % len(result["tokens"]))
...
def main(argv):
    ...
    raw = (os.environ.get("EXTRA_ARGS", "") + " "
           + os.environ.get("SGLANG_EXTRA_ARGS", "")).strip()
    result = normalize(raw)
    if "--json" in argv:
        sys.stdout.write(json.dumps(result, indent=2))
    elif "--argv-nul" in argv:
        # Production startup.sh consumes this: NUL-separated normalized tokens.
        # The trailing NUL is REQUIRED -- without it, bash's
        #   while IFS= read -r -d '' tok; do ...; done
        # silently drops the last token because `read -d ''` only returns
        # success when it has consumed a delimiter. Emit one delimiter per
        # token so every token completes a successful read.
